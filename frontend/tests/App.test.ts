...
beforeEach(() => {
  ...
  api.setReadingPath.mockImplementation(async (_project, _repository, _id, paths, position, headCommitSha) =>
    ({ paths, position: position ?? 0, headCommitSha: headCommitSha ?? null, updatedAt: null }))
...
describe('PR review', () => {
  ...
  it('offers the AI ranking as a proposal and writes the reading path only once accepted', async () => {
    ...
    await flushPromises()
    expect(api.setReadingPath).toHaveBeenCalledWith('project', 'repo-a', 123, ['/src/second.cs', '/src/first.cs'], 0, null)
    expect(wrapper.find('.critical-proposal').exists()).toBe(false)
  ...
  it('leaves a reading path the user already built alone', async () => {
    api.fileReviews.mockResolvedValue({
      files: [], readingPath: { paths: ['/tests/third.cs'], position: 0, headCommitSha: null, updatedAt: null }, updatedAt: null,
    })
    api.generateSummary.mockResolvedValue({
      schemaVersion: 2,
      summary: 'Zmieniono przepływ faktur. Dodano testy.',
